...
from providers import CFClient, SheetClient, Status, StorageClient
...
def guide_creator(event, context):
    ...
    guide_rows, arrays = sheets_client.read_spreadsheet()
    if guide_rows is not None and len(guide_rows) > 0:
        try:
            ...
            for guide_row in guide_rows:
                ...
                for experience_uid in arrays[i]["experience_uids"]:
                    ...
                    experience_object = experience[0].to_dict()  # Only one
                ...
                # Create Image array
                images = cdn_client.upload_image_from_url(
                    guide_row["image_url"], guide_row["title"]
                )
            ...
            n_guides = len(guide_rows)
            msg = f"🆕 New {n_guides} Guides Available"
